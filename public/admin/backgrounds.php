            ...
            if (!empty($_FILES['image']['tmp_name']) && !empty($industry)) {
                ...
                if (!in_array($ext, ['jpg', 'jpeg', 'png', 'webp'])) {
                    ...
                    $existingCount = $db->fetchColumn(
                        "SELECT COUNT(*) FROM background_images WHERE industry = ?",
                        [$industry]
                    );
                    ...
                    if (move_uploaded_file($file['tmp_name'], $targetDir . $filename)) {
                        $db->execute(
                            "INSERT INTO background_images (industry, filename, display_name, sort_order, is_active) VALUES (?, ?, ?, ?, 1)",
...
foreach ($images as $img) {
    $imagesByIndustry[$img['industry']][] = $img;
}
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
        <h3 class="font-semibold text-slate-800 dark:text-white mb-4">
            <i class="fas fa-chart-bar text-primary-500 mr-2"></i>Übersicht
        </h3>
        <div class="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
                <p class="text-2xl font-bold text-primary-600"><?= $stats['industries'] ?></p>
                <p class="text-xs text-slate-500">Branchen</p>
            </div>
        </div>
    </div>
    ...
    <div class="lg:col-span-2 bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
        <h3 class="font-semibold text-slate-800 dark:text-white mb-4">
            <i class="fas fa-upload text-primary-500 mr-2"></i>Neues Bild hochladen
        </h3>
        <form method="POST" enctype="multipart/form-data" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex-1 min-w-[150px]">
                ...
                <input type="text" name="display_name" placeholder="z.B. Modern & Hell"
                       class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-800 dark:text-white">
            </div>
            ...
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="file" name="image" accept=".jpg,.jpeg,.png,.webp" required
                       class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-800 dark:text-white text-sm">
            </div>
    ...
    <div class="p-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
        <h3 class="font-semibold text-slate-800 dark:text-white">
            ...
            <span class="text-sm font-normal text-slate-500 ml-2">
                (<?= count($imagesByIndustry[$industryKey] ?? []) ?> Bilder)
            </span>
        </h3>
    </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            ...
            <div class="relative group rounded-lg overflow-hidden border-2 <?= $img['is_default'] ? 'border-primary-500' : 'border-slate-200 dark:border-slate-600' ?>">
                ...
                <div class="aspect-video bg-slate-100 dark:bg-slate-700">
                    <img src="/assets/backgrounds/<?= e($img['industry']) ?>/<?= e($img['filename']) ?>" 
                         alt="<?= e($img['display_name']) ?>"
                         class="w-full h-full object-cover <?= !$img['is_active'] ? 'opacity-50' : '' ?>">
                </div>
                ...
                <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-2">
                    ...
                    <form method="POST" class="inline">
                        ...
                        <input type="hidden" name="industry" value="<?= $img['industry'] ?>">
                        <button type="submit" class="p-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg" title="Als Standard">
                    ...
                    <form method="POST" class="inline">
                        ...
                        <input type="hidden" name="image_id" value="<?= $img['id'] ?>">
                        <button type="submit" class="p-2 <?= $img['is_active'] ? 'bg-amber-600 hover:bg-amber-700' : 'bg-green-600 hover:bg-green-700' ?> text-white rounded-lg" 
                                title="<?= $img['is_active'] ? 'Deaktivieren' : 'Aktivieren' ?>">
                    ...
                    <form method="POST" class="inline" onsubmit="return confirm('Wirklich löschen?')">
                        ...
                        <input type="hidden" name="image_id" value="<?= $img['id'] ?>">
                        <button type="submit" class="p-2 bg-red-600 hover:bg-red-700 text-white rounded-lg" title="Löschen">
                ...
                <div class="p-2 bg-white dark:bg-slate-800">
                    ...
                    <div class="flex items-center gap-2 mt-1">
                        <?php if ($img['is_default']): ?>
                        <span class="text-xs px-1.5 py-0.5 bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300 rounded">Standard</span>
                        <?php endif; ?>
                        <?php if (!$img['is_active']): ?>
                        <span class="text-xs px-1.5 py-0.5 bg-slate-100 text-slate-600 dark:bg-slate-600 dark:text-slate-300 rounded">Inaktiv</span>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
